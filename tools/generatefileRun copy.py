...
from tools.generatefileRun_core import load_csv

class GeneratefileRun(tk.Frame):
    def __init__(self, parent):
        super().__init__(parent)

        tk.Label(self, text="SampleSheet run Validator",
                 font=("Arial", 15, "bold")).pack(pady=10)
        tk.Label(self, text="1) Upload Slims Extraction, 2) Upload Slims SampleSheet, 3) write Run Name",
                 font=("Arial", 11)).pack(pady=5)
        
        
        #2) Load xlsx file
        tk.Button(self, text="Load Slims Extraction xlsx file",
                  width=35, command=self.load_excel).pack(pady=5)

        #3) Load csv file
        tk.Button(self, text="Load SampleSheet csv file",
                  width=35, command=load_csv(self)).pack(pady=5)

        #4) insert Run name
        options_frame = tk.LabelFrame(self, text="Options", padx=10, pady=10)
        options_frame.pack(fill="x", padx=10, pady=10)

        tk.Label(options_frame, text="Run Name:").grid(
            row=1, column=0, sticky="w")
        self.hpo_entry = tk.Entry(options_frame, width=30)
        self.hpo_entry.grid(row=1, column=1, padx=5, pady=5)
